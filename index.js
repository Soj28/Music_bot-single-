...
function getYouTubeLinkType(value) {
  if (typeof value !== "string") {
    return null;
  }
  ...
  try {
    ...
    const listId = url.searchParams.get("list")?.toUpperCase();

    if (listId?.startsWith("RD")) {
      return "youtube-mix-dynamic";
    }

    if (
      listId?.startsWith("PL") ||
      listId?.startsWith("UU") ||
      listId?.startsWith("OL") ||
      listId?.startsWith("LL") ||
      listId?.startsWith("FL") ||
      listId?.startsWith("OLAK5UY")
    ) {
      return "youtube-playlist";
    }

    return url.searchParams.has("v") ? "youtube-video" : "youtube-url";
  } catch {
    return null;
  }
}

function logPlayRequest(guild, query, userId) {
  const isTrack = typeof query === "object" && query !== null;
  const title = isTrack ? getTrackTitle(query) : String(query || "");
  const url = isTrack ? getTrackUrl(query) : query;
  const linkType = typeof url === "string" ? getYouTubeLinkType(url) : null;

  console.log(
    `[PLAY REQUEST] guild=${guild?.id || "unknown"} user=${userId || "unknown"} type=${linkType || "track"} title=${title} url=${url || "N/A"}`,
  );
}
...
function isPlayableTrack(track) {
  ...
  if (
    ...
    track.id.trim() &&
    track.id.toLowerCase() !== "undefined" &&
    track.id.toLowerCase() !== "null"
  ) {
    return true;
  }
...
async function clearPlaybackHistory(guildId) {
  ...
  // Let the command that caused queueEnd finish its interaction first.
  await new Promise((resolve) => setTimeout(resolve, 1000));

  for (const [key, replies] of ephemeralReplies) {
...
async function ensurePlayer(guild, voiceChannel, textChannel, options = {}) {
  ...
  if (!player || player.destroyed) {
    player = await manager.create(guild.id, {
      volume: volumeState.get(guild.id) ?? 80,
...
function buildVolumeMenu(guildId) {
  const current = volumeState.get(guildId) ?? 80;
...
function buildButtons(guildId) {
  ...
  return new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder()
      ...
      .setLabel(
        loop === "queue" ? "Queue" : loop === "track" ? "Track" : "Loop",
      )
      .setEmoji("🔁")
...
function buildPlayerEmbed(guildId) {
  ...
  if (!track) {
    return embed.setDescription("🎶 Chưa có bài hát nào đang phát.").addFields(
      {
        name: "📡 Trạng thái",
        value: status,
        inline: true,
  ...
  const safeDuration =
    duration && duration !== "0:00"
      ? duration
      : progress.totalMs > 0
        ? formatTime(progress.totalMs)
        : "0:00";

  const progressBar = buildProgressBar(progress.currentMs, progress.totalMs);

  embed.setDescription(url ? `[${title}](${url})` : `**${title}**`).addFields(
    {
      name: "⏱️ Tiến độ",
    ...
    {
      name: "🔊 Volume",
      value: `\`${volumeState.get(guildId) ?? 80}%\``,
      inline: true,
    },
    {
      name: "📡 Trạng thái",
...
async function updateUI(guildId) {
  ...
  try {
    ...
    if (error?.code === 10008 || error?.code === 10003) {
      ...
      console.warn(`[UI UPDATE SKIPPED] Discord timeout guild=${guildId}`);
    } else {
...
async function playQuery({
  ...
  logPlayRequest(guild, query, userId);

  if (typeof query === "object" && !isPlayableTrack(query)) {
    console.error("[PLAY REJECTED] Track không có URL/ID hợp lệ.");
    return null;
  }

  if (typeof query === "string" && isValidUrl(query) === false) {
    console.error(`[PLAY REJECTED] Query không phải URL hợp lệ: ${query}`);
    return null;
  }
  ...
  return withGuildLock(guild.id, async () => {
    ...
    const playbackQuery =
      typeof query === "string" ? normalizePlaybackUrl(query) : query;
    ...
    try {
      await player.play(playbackQuery, userId);
    } catch (error) {
      console.error(
        `[PLAY QUERY ERROR] guild=${guild.id} title=${
          typeof query === "object" ? getTrackTitle(query) : query
        }`,
        error,
      );
      return null;
...
const slashCommands = [
  ...
  new SlashCommandBuilder()
    .setName("leave")
    .setDescription("Cho bot rời khỏi server này"),
  ...
  try {
    ...
    if (
      ...
      await withGuildLock(guildId, async () => {
        if (selected === "none") {
          ...
          if (selected === "bassboost" || selected === "trebleboost") {
            filters.push("limiter");
          }

          await player.filter.applyFilters(filters);
        }
    ...
    if (
      ...
      if (!player) {
        return safeReplyEphemeral(
          interaction,
          "❌ Không tìm thấy link phát hợp lệ cho bài hát này.",
        );
      }
    ...
    if (command === "ui") {
      ...
      return interaction.editReply("✅ Đã hiển thị lại bảng điều khiển nhạc.");
    }
    ...
    if (command === "p") {
      ...
      if (isValidUrl(query)) {
        ...
        if (!player) {
          return interaction.editReply(
            "❌ Không tìm thấy link phát hợp lệ cho yêu cầu này.",
          );
        }
    ...
    if (command === "leave") {
      await interaction.reply({
        content: "👋 Bot sẽ rời khỏi server này.",
        flags: MessageFlags.Ephemeral,
      });

      cancelAutoVoiceRoom(interaction.guildId);
...
client.on("messageCreate", async (message) => {
  ...
  return message
    .reply("⚠️ Lệnh cũ đã bị tắt. Vui lòng dùng các slash command của bot.")
    .catch(() => {});
});
...
manager.on("trackStart", async (player, track) => {
  ...
  if (!isPlayableTrack(track)) {
    console.error(
      `[INVALID TRACK] Bỏ qua track không có URL/ID hợp lệ: ${getTrackTitle(track)}`,
    );
  ...
  console.log(
    `[PLAY STARTED] guild=${guildId} title=${getTrackTitle(track)} url=${getTrackUrl(track) || "N/A"} duration=${getTrackDuration(track, player)}`,
  );
...
manager.on("queueEnd", async (player) => {
  ...
  if (getLoopMode(guildId) === "off" && !autoplayState.get(guildId)) {
    await clearPlaybackHistory(guildId);
  }
