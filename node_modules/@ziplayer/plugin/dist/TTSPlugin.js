...
import { getTTSUrls, getEdgeTTS, getEdgeTTSVoices } from "@ziplayer/tts";
...
export class TTSPlugin extends BasePlugin {
    ...
    async analyzeTTSDuration(text, lang, slow) {
        try {
            ...
            const parts = await Promise.all(urls.map((u) => axios.get(u, { responseType: "arraybuffer" }).then((r) => Buffer.from(r.data))));
    ...
    async getStream(track) {
        ...
        try {
            ...
            const voice = voicelang.length > 0 ?
                voicelang[0].Name
