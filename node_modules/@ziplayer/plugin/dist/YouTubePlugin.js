...
export class YouTubePlugin extends BasePlugin {
    ...
    debug(message, ...optionalParams) {
        ...
        if (this.options.debug)
            this.options.debug(`[YouTubePlugin] ${message}`, ...optionalParams);
    }
    ...
    buildTrack(raw, requestedBy, extra) {
        ...
        const duration = typeof durationValue === "string" && /^\d+:\d{2}(?::\d{2})?$/.test(durationValue.trim())
            ? durationValue.trim().split(":").reduce((total, part) => total * 60 + Number(part), 0) * 1000
            : Number(durationValue) * 1000;
    ...
    canHandle(query) {
        ...
        if (q.startsWith("youtube:") || q.startsWith("yt:"))
            return true;
        ...
        if (this.validate(query)) {
                ...
                if (this.isMixListId(listId)) {
                    ...
                    if (anchorVideoId) {
                        try {
                            ...
                            const info = await this.client.getInfo(anchorVideoId, { client: this.options.searchClientType || "WEB" });
            ...
            if (videoId) {
                try {
                    ...
                    const info = await this.client.getInfo(videoId, { client: this.options.searchClientType || "WEB" });
            ...
            const res = await this.client.search(videoId || query, {
                type: "video",
            });
            const items = res?.items || res?.videos || res?.results || [];
            const tracks = items.slice(0, this.options.searchLimit ?? 10).map((v) => this.buildTrack(v, requestedBy));
            return { tracks };
        }
        // Rest of the method for non-URL queries...
        if (this.canHandle(query) === false)
            return { tracks: [] };
    ...
    async downloadWithYoutubei(track, id, signal) {
    ...
    async downloadWithSabr(track, id, signal) {
    ...
    async getSabrDL(track, id, signal) {
        this.throwIfAborted(signal);
     ...
     */
    async getRelatedTracks(track, opts = {}) {
        await this.ready;
        ...
        const relatedfilter = related.filter((tr) => tr?.content_type === "VIDEO" && !historyUrls.has(tr?.url));
        const reSearchTrack = async (track) => {
            const info = await this.client.getInfo(track?.id ??
                track?.video_id ??
                track?.videoId ??
                track?.content_id ??
                track?.identifier ??
                ...
                track?.basic_info?.content_id, { client: this.options.searchClientType || "WEB" });
            if (info && info.basic_info) {
        ...
        return (await Promise.all(relatedfilter.slice(offset, offset + limit).map((v) => reSearchTrack(v)))).filter((t) => t !== null);
    }
    /**
