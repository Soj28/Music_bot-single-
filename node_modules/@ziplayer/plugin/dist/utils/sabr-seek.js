 ...
 */
async function readWebmInitialization(reader, signal) {
    const chunks = [];
    let total = 0;
    while (total < 1024 * 1024) {
        ...
        if (clusterOffset >= 0) {
            if (clusterOffset === 0)
                throw new Error("SABR bootstrap stream did not contain a WebM initialization segment");
            return data.subarray(0, clusterOffset);
        }
    }
    throw new Error("Could not locate the WebM Cluster boundary in SABR bootstrap stream");
}
function createSabrConfig(formats, serverAbrStreamingUrl, ustreamerConfig, poToken, innertube) {
    const clientName = Constants.CLIENT_NAME_IDS[innertube.session.context.client.clientName];
    if (clientName === undefined)
        throw new Error("Unknown InnerTube client");
    return {
        formats,
 ...
 */
export async function createSabrSeekStream(videoId, innertube, position, signal) {
    if (!videoId)
        throw new Error("videoId is required");
    if (!Number.isFinite(position) || position < 0)
        throw new Error("Invalid SABR seek position");
    if (signal?.aborted)
        throw signal.reason ?? new DOMException("The operation was aborted", "AbortError");
    ...
    try {
        ...
        if (videoInfo.playability_status?.status && videoInfo.playability_status.status !== "OK") {
            throw new Error(`Video is not playable: ${videoInfo.playability_status.status} (${videoInfo.playability_status.reason ?? "no reason given"})`);
        }
        const streamingUrl = videoInfo.streaming_data?.server_abr_streaming_url;
        if (!streamingUrl)
            throw new Error("This video has no SABR streaming URL");
        ...
        const ustreamerConfig = videoInfo.player_config?.media_common_config?.media_ustreamer_request_config?.video_playback_ustreamer_config;
        if (!ustreamerConfig)
        ...
        const formats = adaptiveFormats.map((format) => buildSabrFormat(format));
        ...
        if (position > 0) {
            ...
            if (!bootstrapResult.audioStream)
                throw new Error("SABR bootstrap did not return an audio stream");
        ...
        reloadHandler = async (reloadContext) => {
            try {
                throwIfAborted();
                const reloaded = await innertube.getBasicInfo(videoId, { client: "YTMUSIC" });
                throwIfAborted();
                const newUrl = reloaded.streaming_data?.server_abr_streaming_url
                    ...
                    : undefined;
                const newConfig = reloaded.player_config?.media_common_config?.media_ustreamer_request_config?.video_playback_ustreamer_config;
                const newFormats = (reloaded.streaming_data?.adaptive_formats ?? []).map((format) => buildSabrFormat(format));
        ...
        sabrInstance.buildRequestBody = (abrState, ...args) => {
            if (firstRequest) {
                firstRequest = false;
...
//# sourceMappingURL=sabr-seek.js.map