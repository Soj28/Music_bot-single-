...
function getCachedMinter() {
    const mng = getManager();
    if (!mng)
        return undefined;
    const cached = mng.cache.get(MINTER_CACHE_KEY);
    return cached?.minter;
}
function setCachedMinter(minter) {
    const mng = getManager();
    if (!mng)
        return;
    mng.cache.set(MINTER_CACHE_KEY, { minter });
}
function clearCachedMinter() {
    const mng = getManager();
    if (!mng)
        return;
...
async function createMinter() {
    ...
    if (!ytConfig)
        throw new Error("Could not find ytcfg in YouTube page HTML");
    ...
    if (!initialAttestationData)
        throw new Error("Could not find BotGuard challenge in YouTube page HTML");
    ...
    if (!interpreterUrl)
        throw new Error("Could not get BotGuard interpreter URL");
 ...
 */
export async function mintYouTubePoToken(videoId, signal) {
    if (!videoId)
        throw new Error("videoId is required for a WebPO token");
...
export function resetYouTubeBotGuard() {
    minterPromise = undefined;
    clearCachedMinter();
}
//# sourceMappingURL=youtube-botguard.js.map