...
__export(index_exports, {
  ...
  PlayerActionPriority: () => PlayerActionPriority,
  PlayerBus: () => PlayerBus,
  PlayerBusLatencyTrace: () => PlayerBusLatencyTrace,
  ...
  REQUIRED_PLAYER_QUERIES: () => REQUIRED_PLAYER_QUERIES,
  SEARCH_RPC_TYPES: () => SEARCH_RPC_TYPES,
  ...
  getPlayer: () => getPlayer,
  normalizeTrackMiddleware: () => normalizeTrackMiddleware
});
...
var import_voice9 = require("@discordjs/voice");
...
var PlaybackController = class {
  ...
  constructor(o) {
    ...
    if (this.bus) {
      ...
      this.detachQueries.push(
        ...
        this.bus.registerQuery("isPlaying", () => this.status === import_voice.AudioPlayerStatus.Playing),
        this.bus.registerQuery("isPaused", () => this.status === import_voice.AudioPlayerStatus.Paused),
        this.bus.registerQuery("isIdle", () => this.status === import_voice.AudioPlayerStatus.Idle),
        this.bus.registerQuery("isBuffering", () => this.status === import_voice.AudioPlayerStatus.Buffering),
    ...
    this.onStateChange = (a, b) => {
      ...
      if (b.status === import_voice.AudioPlayerStatus.Buffering) this.armStuckWatchdog();
      else this.clearStuckWatchdog();
      if (b.status === import_voice.AudioPlayerStatus.Idle && a.status !== import_voice.AudioPlayerStatus.Idle) {
  ...
  armStuckWatchdog() {
    ...
    this.stuckTimer = setTimeout(() => {
      ...
      if (this.resourceRefreshInProgress || this.status !== import_voice.AudioPlayerStatus.Buffering || this.activeResource !== resource || this.activeSession !== session)
        return;
  ...
  endResourceRefresh() {
    ...
    if (this.status === import_voice.AudioPlayerStatus.Buffering) this.armStuckWatchdog();
  }
  ...
  createResource(stream, track, inputType) {
    ...
    return (0, import_voice.createAudioResource)(stream, {
      metadata: track,
      inlineVolume: true,
  ...
  play(resource, session, from, to) {
    ...
    if (plan?.enabled && this.activeResource && this.audioPlayer.state.status !== import_voice.AudioPlayerStatus.Idle) {
...
var StreamController = class {
    ...
    if (info.url) {
      try {
        if (/^https?:\/\//i.test(info.url)) {
          ...
          if (!session.isActive()) throw this.abortError();
          return import_stream.Readable.fromWeb(response.body);
        }
  ...
  createResource(stream, track, inputType) {
    const resource = (0, import_voice2.createAudioResource)(stream, {
      metadata: track ?? null,
      inlineVolume: true,
      ...inputType ? { inputType } : {}
    });
...
var FilterController = class {
  ...
  async applyFiltersAndSeek(streamInfo, position = -1) {
    ...
    if (hasSeek && streamInfo.recreate) {
      ...
      const result2 = { ...streamInfo, stream: recreated, url: void 0, inputType: import_voice3.StreamType.Raw, wasRecreated: true };
    ...
    const executable = candidates.find((path) => {
      ...
      return import_node_fs.default.existsSync(path);
    }) || "ffmpeg";
    ...
    const inputType = hasSeek ? import_voice3.StreamType.Raw : import_voice3.StreamType.OggOpus;
    ...
    const proc = (0, import_child_process.spawn)(executable, args, { stdio: ["pipe", "pipe", "ignore"], windowsHide: true });
...
var ConnectionController = class {
  ...
  get isReady() {
    return this.connection?.state.status === import_voice4.VoiceConnectionStatus.Ready;
  }
  get isSubscribed() {
  ...
  setAudioPlayer(audioPlayer) {
    ...
    if (this.connection && this.connection.state.status === import_voice4.VoiceConnectionStatus.Ready) {
  ...
  ensureSubscription(connection = this.connection) {
    ...
    if (connection.state.status !== import_voice4.VoiceConnectionStatus.Ready) {
      return null;
    }
  ...
  async connect(event) {
    ...
    try {
      if (this.connection && this.channel?.id === event.channel.id && this.connection.state.status === import_voice4.VoiceConnectionStatus.Ready) {
        this.ensureSubscription(this.connection);
      ...
      this.connection = null;
      const existing = (0, import_voice4.getVoiceConnection)(this.guildId);
      if (existing) existing.destroy();
      const connection = (0, import_voice4.joinVoiceChannel)({
      ...
      connection.on(import_voice4.VoiceConnectionStatus.Ready, () => {
        if (this.connection !== connection) return;
        this.ensureSubscription(connection);
      });
      connection.on(import_voice4.VoiceConnectionStatus.Disconnected, async () => {
        if (this.connection !== connection) return;
        try {
          await Promise.race([
            (0, import_voice4.entersState)(connection, import_voice4.VoiceConnectionStatus.Signalling, 5e3),
            (0, import_voice4.entersState)(connection, import_voice4.VoiceConnectionStatus.Connecting, 5e3)
          ]);
        } catch {
          if (this.connection === connection) connection.destroy();
        }
      });
      connection.once(import_voice4.VoiceConnectionStatus.Destroyed, () => {
        if (this.connection !== connection) return;
        this.cleanupSubscription();
        this.connection = null;
        this.bus.emitOutput({
      ...
      await (0, import_voice4.entersState)(connection, import_voice4.VoiceConnectionStatus.Ready, this.readyTimeoutMs);
  ...
  constructor(options) {
    ...
    this.unsubscribe.push(
      ...
      this.bus.subscribe("stateChanged", (_event) => {
        ...
        this.isPlaying = status === import_voice5.AudioPlayerStatus.Playing;
        if (status !== import_voice5.AudioPlayerStatus.Idle) this.clearLeaveTimeout();
      }),
  ...
  healthStatus() {
    ...
    return {
      ...
      details: {
        ...
        audioPlayerState: this.player.isPlaying ? import_voice6.AudioPlayerStatus.Playing : this.player.isPaused ? import_voice6.AudioPlayerStatus.Paused : import_voice6.AudioPlayerStatus.Idle
      }
    };
  }
  get playbackMode() {
...
var TTSController = class {
  ...
  constructor(options) {
    ...
    this.ttsPlayer = new import_voice7.AudioPlayer();
    this.onError = (error) => {
  ...
  async playInternal(track) {
    ...
    const wasPlaying = this.audioPlayer?.state.status === import_voice7.AudioPlayerStatus.Playing;
    let started = false;
    try {
      const streamInfo = await this.resolve(track);
      const stream = streamInfo.stream;
      const resource = (0, import_voice7.createAudioResource)(stream, { metadata: track, inlineVolume: true });
      this.activeResource = resource;
      ...
      if (this.ttsPlayer.state.status === import_voice7.AudioPlayerStatus.Playing) {
      ...
      if (this.audioPlayer) {
        ...
        if (wasPlaying && this.audioPlayer.state.status === import_voice7.AudioPlayerStatus.Paused) {
  ...
  waitForPlayingOrIdle() {
    ...
    if (status === import_voice7.AudioPlayerStatus.Playing || status === import_voice7.AudioPlayerStatus.Idle) {
      return Promise.resolve();
    }
    return new Promise((resolve) => {
      const onState = (_oldState, newState) => {
        if (newState.status === import_voice7.AudioPlayerStatus.Playing || newState.status === import_voice7.AudioPlayerStatus.Idle) {
  ...
  waitForIdle(track) {
    if (this.ttsPlayer.state.status === import_voice7.AudioPlayerStatus.Idle) return Promise.resolve();
    ...
    return new Promise((resolve) => {
      ...
      const onState = (_oldState, newState) => {
        if (newState.status === import_voice7.AudioPlayerStatus.Idle) {
          cleanup();
          resolve();
        }
      };
...
var StreamManager = class extends import_events.EventEmitter {
...
var import_voice8 = require("@discordjs/voice");
var PreloadManager = class {
  ...
  async executePreload(track, abortController) {
    ...
    try {
      ...
      const resource = (0, import_voice8.createAudioResource)(preloadInput, {
        inlineVolume: true,
...
function getContentQualityScore(track) {
  ...
  if (author.includes("vevo") || author.includes("official") || author.includes("topic") || author.includes("records") || author.includes("music")) {
    score += 30;
  }
  const duration = track.duration || 0;
  if (duration > 12 * 60 * 1e3) score -= 150;
  if (duration < 60 * 1e3 && !title.includes("interlude")) score -= 100;
  return score;
}
...
var PluginManager = class {
  ...
  pluginRegistrationIndex = /* @__PURE__ */ new Map();
  ...
  pendingSearches = /* @__PURE__ */ new Map();
  // Dedupe search requests
  streamManager;
  constructor(playerOrOptions, manager, options) {
    const isLegacyCall = playerOrOptions && (manager !== void 0 || typeof playerOrOptions === "object" && ("guildId" in playerOrOptions || "bus" in playerOrOptions));
    if (isLegacyCall) {
      this.player = playerOrOptions ?? void 0;
      this.manager = manager ?? void 0;
      this.options = {
        maxFallbackAttempts: 3,
        enableCache: true,
        searchMinScore: 30,
        searchCacheTTL: 2 * 60 * 1e3,
        // 2 minutes
        ...options
      };
    } else {
      const opts = playerOrOptions || {};
      this.player = void 0;
      this.manager = void 0;
      this.options = {
        maxFallbackAttempts: 3,
  ...
  setCachedSearch(query, requestedBy, result) {
    ...
    this.searchCache.set(key, {
      result,
      timestamp: Date.now(),
      expiresAt: Date.now() + (this.options.searchCacheTTL ?? 2 * 60 * 1e3)
    });
  ...
  async searchInternal(query, requestedBy) {
    const timeoutMs = this.options.extractorTimeout ?? 15e3;
  ...
  getPriorityGroupsInfo() {
    ...
    return Array.from(groups.entries()).map(([priority, plugins]) => ({
      priority,
      plugins,
  ...
  async getStreamInternal(track, primary, fresh = false) {
    ...
    const tryPlugin = async (plugin, isPrimary = false) => {
      ...
      if (plugin.getFallback) {
        try {
          ...
          if (result?.stream) {
            const similarity2 = this.calculateTrackSimilarity(track, {
              title: result.metadata?.title || result.metadata?.originalTitle || track.title
            });
            this.debug(`[Stream] ${plugin.name} fallback success (${similarity2})`);
            return {
              result,
    ...
    for (const plugin of fallbackPlugins) {
      ...
      const { result, similarity: similarity2 } = await tryPlugin(plugin);
      if (!result?.stream) {
        continue;
      }
      if (similarity2 >= 0.7) {
        this.debug(`[Stream] Success via fallback ${plugin.name} (score: ${similarity2})`);
      ...
      this.debug(`[Stream] ${plugin.name} low similarity match (${similarity2})`);
    }
  ...
  async getRelatedTracks(track, context) {
    ...
    const history = (Array.isArray(context) ? context : context?.history ? context.history : this.player?.previousTracks) || [];
    const historyUrls = new Set(history.map((t) => t.url));
    const recentAuthors = new Set(
      history.slice(-5).map((t) => normalize(t.author || t.metadata?.author || "")).filter(Boolean)
    );
    ...
    for (const t of results) {
      ...
      if (!unique.has(t.url) && t.url !== currentTrackUrl && !historyUrls.has(t.url)) {
        t._isRecentAuthor = isRecentlyPlayedAuthor;
        unique.set(t.url, t);
      }
    }
    const ranked = Array.from(unique.values()).map((t) => {
      let score = scoreTrack(track, t);
      if (t._isRecentAuthor) score -= 50;
      return { track: t, score };
    }).filter((item) => item.score >= minSimilarityScore).sort((a, b) => b.score - a.score).slice(0, limit).map((x) => x.track);
    this.debug(`[RelatedTracks] Found ${ranked.length} related tracks`);
    return ranked;
  }
  async getVideo(track, options = {}) {
    ...
    const candidates = [
      primary,
      ...this.getAll().filter((plugin) => plugin !== primary && typeof plugin.getVideo === "function").sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0))
    ...
    for (const plugin of candidates) {
      ...
      try {
        ...
        const result = await withTimeout(plugin.getVideo(track, controller.signal), 5e4, `${plugin.name} getVideo timeout`);
        if (result?.stream) {
  ...
  async validateStreamMatchesTrack(streamInfo, expectedTrack) {
    ...
    const similarity2 = this.calculateTrackSimilarity(expectedTrack, { title: actualTitle });
    return similarity2 > 0.6;
  }
  calculateTrackSimilarity(track1, track2) {
    const normalize2 = (str) => str.toLowerCase().replace(/\(.*?\)|\[.*?\]/g, "").replace(/[^a-z0-9\s]/g, "").replace(/\s+/g, " ").trim();
    const title1 = normalize2(track1.title);
    const title2 = normalize2(track2.title || "");
...
var BaseExtension = class extends import_events2.EventEmitter {
  priority;
...
var PlayerRuntimeController = class {
  ...
  initialize(player, manager, options, debug) {
    ...
    const audioPlayer = (0, import_voice9.createAudioPlayer)({ behaviors: { noSubscriber: import_voice9.NoSubscriberBehavior.Pause, maxMissedFrames: 100 } });
    this.audioPlayer = audioPlayer;
...
// src/structures/Player.ts
var Player = class extends import_events3.EventEmitter {
  bus = new PlayerBus();
...
var PlayerManager = class _PlayerManager extends import_events4.EventEmitter {
  debugLevel = "info";
   ...
   */
  cache = new import_lru_cache2.LRUCache({
    max: 500
  });
  SEARCH_CACHE_TTL = 60 * 1e3;
  // 1 minute
